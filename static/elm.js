	...
	F8(
		function (id_, x_, y_, w_, h_, brickIds_, bricks_, polygon_) {
			return {
				...
				height: h_,
				id: id_,
				imgUrl: '/api/piece/' + ($elm$core$String$fromInt(id_) + '.png'),
				outlineUrl: '/api/piece_outline/' + ($elm$core$String$fromInt(id_) + '.png'),
				polygon: polygon_,
				width: w_,
				x: x_,
				y: y_
			...
			if (((_v1.a.$ === 'Just') && (_v1.b.$ === 'Just')) && (_v1.c.$ === 'Just')) {
				...
				return _Utils_update(
					piece,
					{
						bricks: newBrickRefs,
						height: y2 - y,
						imgUrl: '/api/piece/' + ($elm$core$String$fromInt(piece.id) + '.png'),
						outlineUrl: '/api/piece_outline/' + ($elm$core$String$fromInt(piece.id) + '.png'),
						polygon: _List_Nil,
						width: x2 - x,
						x: x,
						y: y
		...
		switch (msg.$) {
			...
			case 'Reset':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{appMode: $author$project$Main$ModeInit, editBrickIds: _List_Nil, editMode: false, editOriginalBrickIds: _List_Nil, generateState: $author$project$Main$NotGenerated, loadState: $author$project$Main$Idle, nextWaveId: 1, pieceGeneration: 0, pieces: _List_Nil, recomputing: false, selectedFileName: '', selectedPieceId: $elm$core$Maybe$Nothing, selectedWaveId: $elm$core$Maybe$Nothing, waves: _List_Nil}),
					$author$project$Main$fetchPdfList);
			case 'LoadFile':
				var path = msg.a;
				return _Utils_Tuple2(
				...
				if (msg.a.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								appMode: $author$project$Main$ModePdf,
			...
			case 'GotMergeResponse':
				if (msg.a.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{appMode: $author$project$Main$ModePieces, generateState: $author$project$Main$Generated, pieceGeneration: model.pieceGeneration + 1, pieces: response.pieces, recomputing: false}),
						A2($elm$core$Task$perform, $author$project$Main$GotViewport, $elm$browser$Browser$Dom$getViewport));
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							...
							function (i, bid) {
								...
								if (_v14.$ === 'Just') {
									...
									return {
										...
										height: brick.height,
										id: newId,
										imgUrl: '/api/piece/' + ($elm$core$String$fromInt(newId) + '.png'),
										outlineUrl: '/api/piece_outline/' + ($elm$core$String$fromInt(newId) + '.png'),
										polygon: _List_Nil,
										width: brick.width,
										x: brick.x,
										y: brick.y
			...
			case 'AssignPieceToGroup':
				var pid = msg.a;
				var _v24 = model.selectedGroupId;
				if (_v24.$ === 'Nothing') {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				} else {
					var gid = _v24.a;
			...
			case 'SetHouseUnitsHigh':
				var s = msg.a;
				var _v32 = $elm$core$String$toFloat(s);
				if (_v32.$ === 'Just') {
					var h = _v32.a;
			...
			case 'StartColorPick':
				...
				var currentHue = _v33.a;
				var currentOpacity = _v33.b;
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					'href',
					piece.imgUrl + ('?v=' + $elm$core$String$fromInt(generation)))
				]),
			_List_Nil);
	});
		...
		var clickMsg = function () {
			if (inGroupAssign) {
				...
				if (inWaveAssign) {
					...
					if (_Utils_eq(appMode, $author$project$Main$ModePieces)) {
						return $author$project$Main$SelectAndEdit(piece.id);
					} else {
...
var $author$project$Main$viewMainSvg = F2(
	function (response, model) {
		...
		var showPieceImages = (_Utils_eq(model.appMode, $author$project$Main$ModePieces) || (_Utils_eq(model.appMode, $author$project$Main$ModeGroups) || _Utils_eq(model.appMode, $author$project$Main$ModeWaves))) && (isGenerated && (!$elm$core$List$isEmpty(model.pieces)));
		...
		var pieceOverlays = ((!model.editMode) && isGenerated) ? A2(
			$elm$core$List$map,
			A8($author$project$Main$viewPieceOverlay, model.appMode, effectiveHoverId, model.selectedPieceId, model.selectedWaveId, model.waves, model.groups, model.selectedGroupId, isLassoing),
			visiblePieces) : _List_Nil;
...
var $author$project$Main$viewWaveTrayThumb = F7(
	function (piece, isLocked, scale, hoveredId, generation, showNum, pos) {
							...
							function (_v1) {
								...
								var thumb = function () {
									...
									if (_v2.$ === 'Just') {
										...
										return _List_fromArray(
											[
												A7($author$project$Main$viewWaveTrayThumb, piece, isLocked, model.svgScale, model.hoveredPieceId, model.pieceGeneration, model.showNumbers, pos)
											]);
									} else {
										return _List_Nil;
									}
			...
			A2(
				...
				$elm$core$List$head(
					A2(
						...
						A2($elm$core$List$indexedMap, $elm$core$Tuple$pair, allGroups)))));
								...
								[
									...
									_List_fromArray(
										[
											...
											$elm$json$Json$Decode$succeed(
												_Utils_Tuple2(
													A2($author$project$Main$MoveGroup, group.id, -1),
													true))),
											$elm$html$Html$Attributes$disabled(!groupIdx),
									...
									_List_fromArray(
										[
											...
											_Utils_cmp(groupIdx, groupCount - 1) > -1),
...
var $author$project$Main$viewGroupThumb = F6(
	function (maybeWaveId, hoveredId, maybeGroup, piece, allIds, generation) {
		var n = $elm$core$List$length(allIds);
		var isHovered = _Utils_eq(
			hoveredId,
			$elm$core$Maybe$Just(piece.id));
		var clickMsg = function () {
			var _v0 = _Utils_Tuple2(maybeGroup, maybeWaveId);
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('group-xn-badge')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(
...
var $author$project$Main$viewWaveRow = F3(
	function (model, allWaves, wave) {
		...
		var countColor = A2($author$project$Main$waveColor, wave.hue, 1.0);
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
				...
				[
					...
					A2(
						...
						A2(
							...
							A2($author$project$Main$toPieceDisplays, model.groups, wave.pieceIds))))
				]));
	});
							...
							[
								...
								_List_fromArray(
									[
										...
										_Utils_Tuple2(
										'active',
										_Utils_eq(model.appMode, $author$project$Main$ModePdf)),
										_Utils_Tuple2('loading', isGenerating)
									])),
								$elm$html$Html$Attributes$disabled((!isLoaded) || (isBusy || isGenerating)),
								$elm$html$Html$Events$onClick(
								$author$project$Main$SetAppMode($author$project$Main$ModePdf))
							]),
						_List_fromArray(
							[
								$elm$html$Html$text(
